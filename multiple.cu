...
kernel5(dtype *g_idata, dtype *g_odata, unsigned int n)
{
  ...
  int num = threads;  
  while((i + num)< n) {
    scratch[threadIdx.x] += g_idata[i + num];
    num+= threads;
  }
  __syncthreads ();
  ...
  for(unsigned int s = blockDim.x / 2; s > 32; s = s >> 1) {
    ...
    __syncthreads ();
  	...
  	if (threadIdx.x < 32) {
		...
		volatile dtype *Unroll = scratch;

		if(n > 64) {
			Unroll[threadIdx.x] += Unroll[threadIdx.x + 32];
		}
		if(n > 32) {
			Unroll[threadIdx.x] += Unroll[threadIdx.x + 16];
		}
		Unroll[threadIdx.x] += Unroll[threadIdx.x + 8];
		Unroll[threadIdx.x] += Unroll[threadIdx.x + 4];
		Unroll[threadIdx.x] += Unroll[threadIdx.x + 2];
		Unroll[threadIdx.x] += Unroll[threadIdx.x + 1];		
	}
